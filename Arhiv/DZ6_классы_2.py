class Student:
    ...
    def average_score(self):                                # Средний балл за домашку одного студента
        total = 0
        total_len = 0
...
def evaluate_persons(in_list, cours)   :                          # Средний балл за курс всех студентов
    a = 0
    for st in in_list:
        if cours in st.courses_in_progress:
            a += st.average_score()
    print(f'Cредняя оценка за домашнее задания по курсу "{cours} по всем студентам: {a/len(in_list)}')


st_list = [student1, student4]
evaluate_persons(st_list, "Python")
